...
def deploy_push(request, project_id, deploy_id):
    ...
    :return:
    """
    deploy_object = models.Deploy.objects.filter(id=deploy_id, project_id=project_id).first()  # 找出符合这个项目的任务
    ...
    def task(host_object, deploy_object):
        ...
        def File():
            ...
            try:
                ...
                # file_name = deploy_object.file.title  # 更新文件的名字，
                file_name = 'include.txt'  # 更新文件的名字，

                # 更新文件的绝对路径
                # /home/yx/file/PHP/20190122154728/file_name
        ...
        def publish():
            ...
            try:
                ...
                file_object = sftp.open(target_file_path, mode='w')  # 在指定目录下打开一个文件，然后写入
                # print('脚本内容',deploy_object.script.code)
                file_object.write(deploy_object.script.code)  # 把数据库里面的脚本内容拿到，表名.字段名
                interpreter_type = deploy_object.script.interpreter  # 得到脚本是什么类型
        ...
        def update_status():
            ...
            if not status:
                ...
                record_object.deploy_time = update_time
